<template>
  <div>
  <div class="bar">
    <input type="text"  placeholder="输入搜索内容" style="margin-right: 30px;"/> 
    <el-button type="primary" @click="openAddDialog">添加信息</el-button>
  </div> 

    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row :total="total">

<el-table-column v-for="column in columns" :key="column.key" :prop="column.key" :label="column.label" align="center" :width="column.key === 'site_updated_at' ? '180px' : column.key === 'domain' ? '200px' : ''">
  <template slot="header" slot-scope="scope">
    <div class="sortable-header" @click="sortTable(column.key)">
      <span>{{ column.label }}</span>
      <span class="ant-table-column-sorter ant-table-column-sorter-full">
      <span class="ant-table-column-sorter-inner">
      <span role="button" aria-label="caret-up" class="anticon anticon-caret-up ant-table-column-sorter-up">
        <svg focusable="false" data-icon="caret-up" width="1em" height="1em" fill="currentColor" aria-hidden="true" viewBox="0 0 1024 1024">
          <path d="M858.9 689L530.5 308.2c-9.4-10.9-27.5-10.9-37 0L165.1 689c-12.2 14.2-1.2 35 18.5 35h656.8c19.7 0 30.7-20.8 18.5-35z"></path>
        </svg>
      <span role="button" aria-label="caret-down" class="anticon anticon-caret-down ant-table-column-sorter-down">
        <svg focusable="false" data-icon="caret-down" width="1em" height="1em" fill="currentColor" aria-hidden="true" viewBox="0 0 1024 1024">
          <path d="M840.4 300H183.6c-19.7 0-30.7 20.8-18.5 35l328.4 380.8c9.4 10.9 27.5 10.9 37 0L858.9 335c12.2-14.2 1.2-35-18.5-35z"></path>
        </svg>
      </span></span></span></span></div>
      </template>
        <template slot-scope="scope">
        <span v-if="!scope.row.editing" @click="onClick(scope.row, column.key)">{{ scope.row[column.key] }}</span>
        <el-input v-else v-model="scope.row[column.key]" size="mini" @blur="onBlur(scope.row, column.key)">
          </el-input>
    </template>
    </el-table-column>
      <el-table-column label="操作" width="160" align="center">

        <el-button type="primary" size="small" @click="handleEdit">修改</el-button>
        <el-dialog title="编辑" :visible="editDialogVisible" @close="editDialogVisible = false">
          <el-form :model="editFormData" ref="editForm" label-position="left">
          <el-row><el-col :span="10"><el-form-item label="域名" prop="domain"><el-input v-model="addFormData.domain"></el-input></el-form-item> </el-col>
          <el-col :span="1"><el-form-item label=""></el-form-item></el-col>
          <el-col :span="5"><el-form-item label="小组" prop="name"><el-input v-model="addFormData.name"></el-input></el-form-item></el-col>
          </el-row>
          <el-row>
          <el-col :span="2"><el-form-item label="星期一" prop="site_monday"><el-input v-model="addFormData.site_monday"></el-input></el-form-item></el-col>
          <el-col :span="1"><el-form-item label=""></el-form-item></el-col>
          <el-col :span="2"><el-form-item label="星期二" prop="site_tuesday"><el-input v-model="addFormData.site_tuesday"></el-input></el-form-item></el-col>
          <el-col :span="1"><el-form-item label=""></el-form-item></el-col>
          <el-col :span="2"><el-form-item label="星期三" prop="site_wednesday"><el-input v-model="addFormData.site_wednesday"></el-input></el-form-item></el-col>
          <el-col :span="1"><el-form-item label=""></el-form-item></el-col>
          <el-col :span="2"><el-form-item label="星期四" prop="site_thursday"><el-input v-model="addFormData.site_thursday"></el-input></el-form-item></el-col>
          <el-col :span="1"><el-form-item label=""></el-form-item></el-col>
          <el-col :span="2"><el-form-item label="星期五" prop="site_friday"><el-input v-model="addFormData.site_friday"></el-input></el-form-item></el-col>
          <el-col :span="1"><el-form-item label=""></el-form-item></el-col>
          <el-col :span="2"><el-form-item label="星期六" prop="site_saturday"><el-input v-model="addFormData.site_saturday"></el-input></el-form-item></el-col>
          <el-col :span="1"><el-form-item label=""></el-form-item></el-col>
          <el-col :span="2"><el-form-item label="星期日" prop="site_sunday"><el-input v-model="addFormData.site_sunday"></el-input></el-form-item></el-col>
          </el-row>
          <el-row><el-col :span="10"><el-form-item label="修改时间" prop="site_updated_at"><el-input v-model="addFormData.site_updated_at"></el-input></el-form-item></el-col></el-row>

        <el-button type="primary" @click="submitEditForm">确认修改</el-button>
          </el-form>
        </el-dialog>




        <el-button type="danger" size="small" @click="handleDelete= false">删除</el-button>
      </el-table-column>
    </el-table>

  <el-dialog title="添加信息" :visible="addDialogVisible" @close="addDialogVisible = false">
  <el-form :model="addFormData" ref="addForm" label-position="left">
    <el-row><el-col :span="10"><el-form-item label="域名" prop="domain"><el-input v-model="addFormData.domain"></el-input></el-form-item> </el-col>
      <el-col :span="1"><el-form-item label=""></el-form-item></el-col>
      <el-col :span="5"><el-form-item label="小组" prop="name"><el-input v-model="addFormData.name"></el-input></el-form-item></el-col>
    </el-row>
    <el-row>
    <el-col :span="2"><el-form-item label="星期一" prop="site_monday"><el-input v-model="addFormData.site_monday"></el-input></el-form-item></el-col>
    <el-col :span="1"><el-form-item label=""></el-form-item></el-col>
    <el-col :span="2"><el-form-item label="星期二" prop="site_tuesday"><el-input v-model="addFormData.site_tuesday"></el-input></el-form-item></el-col>
    <el-col :span="1"><el-form-item label=""></el-form-item></el-col>
    <el-col :span="2"><el-form-item label="星期三" prop="site_wednesday"><el-input v-model="addFormData.site_wednesday"></el-input></el-form-item></el-col>
    <el-col :span="1"><el-form-item label=""></el-form-item></el-col>
    <el-col :span="2"><el-form-item label="星期四" prop="site_thursday"><el-input v-model="addFormData.site_thursday"></el-input></el-form-item></el-col>
    <el-col :span="1"><el-form-item label=""></el-form-item></el-col>
    <el-col :span="2"><el-form-item label="星期五" prop="site_friday"><el-input v-model="addFormData.site_friday"></el-input></el-form-item></el-col>
    <el-col :span="1"><el-form-item label=""></el-form-item></el-col>
    <el-col :span="2"><el-form-item label="星期六" prop="site_saturday"><el-input v-model="addFormData.site_saturday"></el-input></el-form-item></el-col>
    <el-col :span="1"><el-form-item label=""></el-form-item></el-col>
    <el-col :span="2"><el-form-item label="星期日" prop="site_sunday"><el-input v-model="addFormData.site_sunday"></el-input></el-form-item></el-col>
    </el-row>
    <el-row><el-col :span="10">
      <el-form-item label="修改时间" prop="site_updated_at"> 
        <el-date-picker
          v-model="selectedDateTime"
          type="datetime"
          placeholder="选择修改时间"
          format="yyyy-MM-dd HH:mm:ss"
          @change="handleDateTimeChange">
        </el-date-picker></el-form-item></el-col></el-row>
    <el-button type="primary" @click="submitAddForm">确认添加</el-button>
  </el-form>
  </el-dialog>


  <pagination v-show="total>0" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="getList" />
</div>
</template>

<script>
import moment from 'moment';
import { fetchSite } from '@/api/article';
import Pagination from '@/components/Pagination'; 
import { addSite } from '@/api/article';
export default {
  components: { Pagination },
  data() {
    return {
      selectedDateTime: '' ,
      addDialogVisible: false,
      addFormData: {
        domain: '',name: '',
        site_monday: '',site_tuesday: '',site_wednesday: '',site_thursday: '',site_friday: '',site_saturday: '',site_sunday: '',site_updated_at: '',
      },
      editDialogVisible: false,
      editFormData: {site_sunday:'',site_friday:''},

      list: null,
      total: 0,
      listLoading: true,
      listQuery: {
        page: 1,
        limit: 10
      },
      columns: [
        { key: 'domain', label: '域名' },
        { key: 'name', label: '小组' },
        { key: 'site_monday', label: '星期一' },
        { key: 'site_tuesday', label: '星期二' },
        { key: 'site_wednesday', label: '星期三' },
        { key: 'site_thursday', label: '星期四' },
        { key: 'site_friday', label: '星期五' },
        { key: 'site_saturday', label: '星期六' },
        { key: 'site_sunday', label: '星期日' },
        { key: 'site_updated_at', label: '修改时间' }
      ]
    };
  },
  created() {
    this.getList();
  },
  methods: {
  getList() {
      console.log('正在获取数据，当前页码：', this.listQuery.page, '每页数量：', this.listQuery.limit);
  
    this.listLoading = true;
    fetchSite({
    page: this.listQuery.page,
    limit: this.listQuery.limit
     }).then(response => {
          this.total = response.count;
          this.list = response.data;
           // 对所有数据进行排序
      
          const startIndex = (this.listQuery.page - 1) * this.listQuery.limit;
          const endIndex = Math.min(startIndex + this.listQuery.limit, this.total); // 防止超出总数
          this.list = this.list.slice(startIndex, endIndex);
          
          console.log('当前页数据:', this.list);
          this.listLoading = false;
  }).catch(error => {
    console.error('请求错误:', error);
    this.listLoading = false;
      });
  },
  sortTable(key) {
      // 点击排序时的逻辑
      console.log('点击了排序，排序键：', key);
      // 根据键值进行排序
      this.list.sort((a, b) => {
          if (typeof a[key] === 'string') {
              // 如果是字符串类型，按字母顺序排序
              return a[key].localeCompare(b[key]);
          } else if (typeof a[key] === 'number') {
              // 如果是数字类型，按数字大小排序
              return a[key] - b[key];
          } else {
              return 0;
          }
      });

      // 输出排序后的数据
      this.list.forEach(item => {
          console.log('数据：', item[key]);
      });

      // 切换排序方式
      this.listQuery.order = this.listQuery.order === 'asc' ? 'desc' : 'asc';
  },
  openAddDialog() {
    this.addDialogVisible = true;
  },
  submitAddForm() {
    console.log("Submitting add form with data:", this.addFormData);
    addSite(this.addFormData)
      .then(res => {
        console.log("Response from addSite:", res);
      })
      .catch(error => {
        console.error("Error from addSite:", error);
      });
  },
  handleEdit(row) {
  this.editDialogVisible = true;
  // 将当前行数据填充到编辑表单中
  this.editFormData = { ...row };
},
  
  submitEditForm() {
      updateSite(this.editFormData)
        .then(res => {
          console.log("Response from updateSite:", res);
        })
        .catch(error => {
          console.error("Error from updateSite:", error);
        });
    },
    handleDateTimeChange(value) {
      if (value) {
        
        const formattedDateTime = moment(value).format('YYYY-MM-DD HH:mm:ss'); // 使用 Moment.js 格式化
        this.addFormData.site_updated_at = formattedDateTime; // 将格式化后的时间字符串赋值给 site_updated_at
      }
    }


}}
</script>

<style lang="scss" scoped>
.sortable-header {
  display: flex;
  flex-direction: row; /* 将排序按钮水平排列 */
  align-items: center; /* 水平居中对齐 */
  margin-left: 30%;
}

.ant-table-column-sorter-up,
.ant-table-column-sorter-down {
  display: block; /* 将排序箭头显示为块级元素 */
  // margin-top: 2px; /* 适当调整排序箭头距离列名的上方间距 */
  line-height: 1; /* 调整行高 */
  margin-left: 0.5px;
}
.ant-table-column-sorter-down {
  margin-top: -5px; /* 适当调整排序箭头距离列名的上方间距 */
}
  .bar{
        border-radius: 2px;
        width: 400px;
        padding: 14px;
        margin: 1px 2px;
        position:relative;
    }
  .bar input{
        background:#fff no-repeat 13px 13px;
        background-image:url(data:image/png;base64,);

        border: none;
        line-height: 19px;
        padding: 11px 0;

        border-radius: 2px;
        box-shadow: 0 2px 8px #c4c4c4 inset;
        text-align: left;
        font-size: 14px;
        font-family: inherit;
        color: #738289;
        font-weight: bold;
        outline: none;
        text-indent: 40px;
    }
  
</style>
